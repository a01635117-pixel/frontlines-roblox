// Golden "nukes" scenario from the ORIGINAL OpenFrontIO TS.
//
// Usage (from the OpenFrontIO checkout):
//   cd ../OpenFrontIO && npx tsx ../OpenFrontRoblox/tools/golden-nukes.mts
//
// Two scripted humans (Europe, North America) plus 40 bots. Alice builds a
// missile silo and fires atom bombs, a hydrogen bomb, a stacked double
// launch and a MIRV; Bob builds a SAM launcher (intercepts) and a silo and
// strikes back. Intents are recorded with their tick for exact replay.
// Writes tests/NukeVectors.luau (same format as BuildVectors).
...
const SCENARIO = {
  ...
  disabledUnits: ["Factory", "Warship"],
  startingGold: 200_000_000,
};
...
// First tile of the living bot with the most tiles (MIRV / H-bomb target).
function biggestBotTile(): number | null {
  let best: any = null;
  ...
  for (const t of best.tiles()) return t;
  return null;
}
...
function intentsFor(tick: number): Intent[] {
  ...
  if (tick === 205) {
    out.push({ type: "build_unit", clientID: "human1", unit: "Missile Silo", tile: tileA });
    out.push({ type: "build_unit", clientID: "human2", unit: "SAM Launcher", tile: tileB });
  }
  if (tick === 340) {
    // Atom bomb on Bob, inside his SAM's range.
    const t = farTile(B, tileB, 35); // outside the SAM's blast, inside its range
    if (t !== null) out.push({ type: "build_unit", clientID: "human1", unit: "Atom Bomb", tile: t, rocketDirectionUp: true });
  }
  if (tick === 360) {
    const silo = A.units("Missile Silo")[0];
    if (silo) out.push({ type: "upgrade_structure", clientID: "human1", unitId: silo.id(), amount: 1 });
  }
  if (tick === 400) {
    const t = farTile(B, tileB, 20);
    if (t !== null) out.push({ type: "build_unit", clientID: "human2", unit: "Missile Silo", tile: t });
  }
  if (tick === 470) {
    // Hydrogen bomb on the biggest bot, arcing downwards.
    const t = biggestBotTile();
    if (t !== null) out.push({ type: "build_unit", clientID: "human1", unit: "Hydrogen Bomb", tile: t, rocketDirectionUp: false });
  }
  if (tick === 540) {
    // Bob strikes back at Alice.
    const t = farTile(A, tileA, 32) ?? farTile(A, tileA, 20); // outside the blast of her silo when possible
    if (t !== null) out.push({ type: "build_unit", clientID: "human2", unit: "Atom Bomb", tile: t, rocketDirectionUp: true });
  }
  if (tick === 700) {
    // Stacked double launch at Bob (both silo slots).
    const t = farTile(B, tileB, 40);
    if (t !== null) out.push({ type: "build_unit", clientID: "human1", unit: "Atom Bomb", tile: t, rocketDirectionUp: true, amount: 2 });
  }
  if (tick === 900) {
    const t = biggestBotTile();
    if (t !== null) out.push({ type: "build_unit", clientID: "human1", unit: "MIRV", tile: t });
  }
  return out;
}
...
const seenUnits = new Map<number, number>();
...
for (let tick = 0; tick <= SCENARIO.ticks; tick++) {
  ...
  for (const u of game.units()) if (!seenUnits.has(u.id())) { seenUnits.set(u.id(), 1); createdLog.push(`t${tick} ${u.type()}(${u.owner().name()})`); }
}
...
const lua = `-- AUTO-GENERATED by tools/golden-nukes.mts from upstream OpenFrontIO. Do not edit.
...
`;
fs.writeFileSync(path.resolve(here, "../tests/NukeVectors.luau"), lua);
...
console.log(`units seen alive at a tick end: ${createdLog.join(", ")}`);
console.log(`fallout tiles: ${game.numTilesWithFallout()}; MIRVs launched: ${game.stats().numMirvsLaunched()}`);
