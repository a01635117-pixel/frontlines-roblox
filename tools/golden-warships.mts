// Golden "warships" scenario from the ORIGINAL OpenFrontIO TS.
//
// Usage (from the OpenFrontIO checkout):
//   cd ../OpenFrontIO && npx tsx ../OpenFrontRoblox/tools/golden-warships.mts
//
// Two scripted humans (Europe, North America) plus 40 bots. Both build ports
// and trade; Alice builds warships and later sends one across the ocean to
// Bob's port (move_warship) to hunt his trade ships; Bob builds a warship to
// defend. Covers patrolling, shells, trade-ship capture, warship duels and
// repair retreats. Writes tests/WarshipVectors.luau.
...
// Nearest open-water (non-shoreline) tile to `from`, by ring search.
function nearestOpenWater(from: number): number | null {
  const x0 = gameMap.x(from);
  const y0 = gameMap.y(from);
  for (let r = 1; r < 80; r++) {
    for (let dy = -r; dy <= r; dy++) {
      for (let dx = -r; dx <= r; dx++) {
        if (Math.max(Math.abs(dx), Math.abs(dy)) !== r) continue;
        ...
        if (!gameMap.isValidCoord(x, y)) continue;
        const t = gameMap.ref(x, y);
        if (gameMap.isWater(t) && !gameMap.isShoreline(t)) return t;
      }
    }
  }
  return null;
...
function intentsFor(tick: number): Intent[] {
  ...
  if (tick === 400 || tick === 460) {
    const w = nearestOpenWater(tileA);
    if (w !== null) out.push({ type: "build_unit", clientID: "human1", unit: "Warship", tile: w });
  }
  if (tick === 430) {
    const w = nearestOpenWater(tileB);
    if (w !== null) out.push({ type: "build_unit", clientID: "human2", unit: "Warship", tile: w });
  }
  if (tick === 700) {
    // Send Alice's first warship to Bob's port waters.
    const ws = A.units("Warship");
    const w = nearestOpenWater(tileB);
    if (ws.length > 0 && w !== null) out.push({ type: "move_warship", clientID: "human1", unitIds: [ws[0].id()], tile: w });
  }
  if (tick === 1500) {
...
const lua = `-- AUTO-GENERATED by tools/golden-warships.mts from upstream OpenFrontIO. Do not edit.
...
`;
fs.writeFileSync(path.resolve(here, "../tests/WarshipVectors.luau"), lua);
